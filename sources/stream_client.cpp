...
void StreamClient::init(QString login, QString pass)
{
    ...
    connect(uiController.get(), &UiController::sendText, client.get(), &Client::sendMessage, Qt::QueuedConnection);
    connect(client.get(), &Client::messageReceived, uiController.get(), &UiController::addText, Qt::QueuedConnection);
    connect(uiController.get(), &UiController::sendFile, client.get(), &Client::sendFile, Qt::QueuedConnection);
